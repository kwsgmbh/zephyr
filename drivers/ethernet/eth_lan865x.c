...
static int lan865x_port_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	// LOG_INF("Sending packet: buf=%p, len=%zu\n", pkt->cursor.buf, net_pkt_get_len(pkt));
...
int eth_lan865x_send_pkt(struct net_pkt *pkt)
{
	...
	LOG_INF("IFACE VALUE IS >>> %p",iface);
    if (!iface) {
        LOG_ERR("No Ethernet interface found");
        return -ENODEV;
    }

    const struct ethernet_api *eth_api = net_if_get_device(iface)->api;
	LOG_INF("eth api VALUE IS >>> %p",eth_api);
    if (!eth_api || !eth_api->send) {
        LOG_ERR("Ethernet API not available");
        return -ENODEV;
    }
